...
DocPropsApp = (function() {
  ...
  DocPropsApp.prototype.toxml = function() {
    ...
    tmp.ele('vt:variant').ele('vt:lpstr', 'å·¥ä½œè¡¨');
...
Workbook = (function() {
  ...
  Workbook.prototype.save = function(cb) {
    ...
    try {
      fs.copy (opt.tmpl_path + '/tmpl', target, function () {
        fs.mkdirSync (target + '/xl/_rels');
        fs.mkdirSync (target + '/xl/worksheets');
        fs.writeFileSync (target + '/[Content_Types].xml', self.ct.toxml());
        fs.writeFileSync (target + '/docProps/app.xml', self.da.toxml());
        fs.writeFileSync (target + '/xl/workbook.xml', self.wb.toxml());
        fs.writeFileSync (target + '/xl/sharedStrings.xml', self.ss.toxml());
        fs.writeFileSync (target + '/xl/_rels/workbook.xml.rels', self.re.toxml());
        fs.writeFileSync (target + '/xl/styles.xml', self.st.toxml());
        for(var i=0; i<sheetList.length; i++) {
          require ('fs').writeFileSync (sheetList [i].name, sheetList [i].data, 'utf-8');  
        }
        ...
        zip.writeToFile(path.join(self.fpath, self.fname));
          fs.remove(target, function() {
            return cb(null, path.join(self.fpath, self.fname)); // return err, path to file
          });
        });
      });
